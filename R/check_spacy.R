...
check_spacy <- function(which_python = NA) {
    ...
    if (is.na(which_python)) {
        tryCatch({
            all_python <- system2(c("which", "-a", "python"), stdout = T)
        },
        warning = function(e) {
    ...
    if (sum(spacy_found) == 1) {
        ...
        stop(
            paste(
                "More than one python installation with spacy was found",
    ...
    PYTHON_SCRIPT <-
        system.file("python", "posTag.py", package = "spacyr")
    x <- "Sample Text"
    ...
    ret <- ""
    suppressWarnings(ret <-
                         paste0(
                             system2(
                                 paste0(PYTHON_PATH, "python"),
